...
class OpenSegMaskFormerDenseTextKDLora(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # frozen = ['conv1', 'conv2', 'conv3', 'layer1.0.conv1', 'layer1.0.conv2', 'layer1.1.conv2', 'layer1.1.conv3', 'layer1.2.conv2', 'layer1.2.conv3', 'layer2.0.conv1', 'layer2.0.conv2', 'layer2.0.downsample.0', 'layer2.1.conv1', 'layer2.1.conv2', 'layer2.2.conv2', 'layer2.2.conv3', 'layer2.3.conv2', 'layer2.3.conv3', 'layer3.0.conv1', 'layer3.0.conv2', 'layer3.1.conv2', 'layer3.2.conv2', 'layer3.3.conv2', 'layer3.4.conv2', 'layer3.5.conv2', 'layer4.0.conv2', 'layer4.2.conv2']
        # for name, param in self.backbone.named_parameters():
        ...
        attn_shape = ShapeSpec(channels=1024, height=None, width=None, stride=32)
        ...
        if self.kd_super:
            ...
            if self.kd_stu_source == 'prior_relu':
                ...
                self.kd_proj = nn.Linear(in_dim, tec_dim.channels)
                for i in range(dec_layers - 1):
                    aux_kd_proj = nn.Linear(in_dim, tec_dim.channels)
                    setattr(self, "aux_kd_porj_{}".format(i), aux_kd_proj)
    ...
    def from_config(cls, cfg):
        ...
        backbone_out_shape = {'res2': ShapeSpec(channels=256, height=None, width=None, stride=4), 'res3': ShapeSpec(channels=512, height=None, width=None, stride=8), 'res4': ShapeSpec(channels=1024, height=None, width=None, stride=16), 'res5': ShapeSpec(channels=2048, height=None, width=None, stride=32), 'attn': ShapeSpec(channels=1024, height=None, width=None, stride=32)}
        ...
        # import pdb; pdb.set_trace()
        if cfg.MODEL.KD.TEXT_KD_WEIGHT > 0 and kd_type == 'vision':
    ...
    def forward(self, batched_inputs):
        ...
        # import pdb; pdb.set_trace()
        # from fvcore.nn import parameter_count, parameter_count_table, FlopCountAnalysis
        features = self.backbone(images.tensor)
        # images.tensor: (B,C,H,W)
        import pdb; pdb.set_trace()
        # flops11 = FlopCountAnalysis(self.backbone, images.tensor)
        # print(flops11.total())
        # from thop import profile
        # flops1, params = profile(self.backbone, inputs=(images.tensor,))
        # print(flops1)
        # import pdb; pdb.set_trace()

        outputs, seg_feature = self.sem_seg_head(features)
        # from thop import profile
        # flops2, params = profile(self.sem_seg_head, inputs=(features,))
        # print(flops2)
        # import pdb; pdb.set_trace()
        # # loss_band
        ...
        if self.training:
            ...
            if self.kd_super:
                ...
                for i, a_indices in enumerate(aux_indices):
                    ...
                    if self.kd_type == "vanilla":
                        kd_loss = ((aux_kd_v_embed - masked_clip_features) ** 2).sum(dim=1).mean()
            ...
            for k in list(losses.keys()):
                if k in self.criterion.weight_dict:
                    ...
                    losses.pop(k)
            return losses #, cls_pred, images_id
        else:
            # training sample
            # overfit/
