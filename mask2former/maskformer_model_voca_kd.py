...
from .kd_clip import CLIPResNetWithAttention, CLIPVisionTransformer

@META_ARCH_REGISTRY.register()
class OpenSegKDMaskFormer(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # KD
        # self.kd_proj = nn.Linear(256, 1024)
        self.kd_proj = nn.Linear(256, 512)
    ...
    def from_config(cls, cfg):
        ...
        # vision_clip = build_backbone(cfg)
        # vision_clip = CLIPResNetWithAttention(cfg)
        vision_clip = CLIPVisionTransformer(cfg)
        ...
        return {
            ...
            "vit_num_head": cfg.MODEL.VIT.HEADS,
            # inference
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            # kd_clip_features = self.vision_clip(images.tensor)
            ...
            #         masks_list.append(gt_mask)
            #     if not masks_list:
            #         continue
            #     all_gt_masks = torch.stack(masks_list)  # N,1,h,w
            #     masked_clip_features = self.vision_clip.forward_attn(kd_clip_features['res5'][b:b+1], all_gt_masks)
            ...
            #----------------------------------------------
            patch_size, vit_num_head = self.vit_patch_size, self.vit_num_head

            num_object = [targets[b]["masks"].shape[0] for b in range(len(indices))]
            max_num_obj = int(max(num_object))
            img_size = images.tensor.shape[-2:]
            mask_pad = torch.zeros(len(batched_inputs), max_num_obj, img_size[0], img_size[1], device=self.device).float() # shape (b, max_num_obj, h, w)
            for b in range(len(indices)):
                mask_ = targets[b]["masks"].float()
                if mask_.shape[-2:] != img_size:
                    mask_ = F.interpolate(mask_, img_size, mode='nearest')
                mask_pad[b][:num_object[b]] = mask_
            mask_pad = F.max_pool2d(mask_pad, patch_size, stride=patch_size).flatten(-2, -1) # shape (b, max_num_obj, num_img_patch)

            img_patch_num = img_size[0] // patch_size * img_size[1] // patch_size
            patch_num = int(max_num_obj + img_patch_num)
            atten_mask = torch.zeros((len(batched_inputs), patch_num, patch_num), device=self.device).float() # shape (b, max_num_obj + num_img_patch, max_num_obj + num_img_patch)

            # import pdb;pdb.set_trace()

            atten_mask[:, :max_num_obj, max_num_obj:] = mask_pad       # object token, cal attention with object patch
            atten_mask[:, max_num_obj:, max_num_obj:] = 1   # img patch, cal with each others

            # consider multi-head attention
            atten_mask = atten_mask[:, None, :, :].repeat(1, vit_num_head, 1, 1).view(len(batched_inputs) * vit_num_head, patch_num, patch_num) # shape (b * num_head, total_patch, total_patch)
            atten_mask = atten_mask > 0.5

            clip_obj_ebmd = self.vision_clip(images.tensor, atten_mask, max_num_obj)
            ke_ins_clip = torch.cat([i_c[J] for i_c, (_, J) in zip(clip_obj_ebmd, indices)])

            kd_vision_feature = self.kd_proj(vision_feature)
            idx = self.criterion._get_src_permutation_idx(indices)
            kd_ins_emb = kd_vision_feature[idx]

            kd_loss = F.mse_loss(kd_ins_emb, ke_ins_clip)
            losses.update({'loss_kd': kd_loss})
            #----------------------------------------------
