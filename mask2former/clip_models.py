...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # file = open("cka_han.txt", "w")
        out = self.relu(self.bn1(self.conv1(x)))
        # out_str = str(out.tolist())
        # file.write(out_str + "\n")
...
class BottleneckLora(nn.Module):
    expansion = 4
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        # all conv layers have stride 1. an avgpool is performed after the second convolution when stride > 1
        self.conv1 = lora.Conv2d(inplanes, planes, 1, bias=False, r=8)
        self.bn1 = nn.BatchNorm2d(planes)

        self.conv2 = lora.Conv2d(planes, planes, 3, padding=1, bias=False, r=8)
        self.bn2 = nn.BatchNorm2d(planes)

        self.avgpool = nn.AvgPool2d(stride) if stride > 1 else nn.Identity()

        self.conv3 = lora.Conv2d(planes, planes * self.expansion, 1, bias=False, r=8)
        self.bn3 = nn.BatchNorm2d(planes * self.expansion)
        ...
        if stride > 1 or inplanes != planes * BottleneckLora.expansion:
            # downsampling layer is prepended with an avgpool, and the subsequent convolution has stride 1
            self.downsample = nn.Sequential(OrderedDict([
                ("-1", nn.AvgPool2d(stride)),
                ("0", nn.Conv2d(inplanes, planes * self.expansion, 1, stride=1, bias=False)),
...
@BACKBONE_REGISTRY.register()    
class CLIPResNetWithAttentionLora(Backbone):
    """
    A ResNet class that is similar to torchvision's but contains the following changes:
    - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
    ...
    def __init__(self, cfg, input_shape):
        ...
        # the 3-layer stem
        self.conv1 = lora.Conv2d(3, width // 2, kernel_size=3, stride=2, padding=1, bias=False, r=8)
        self.bn1 = nn.BatchNorm2d(width // 2)
        self.conv2 = lora.Conv2d(width // 2, width // 2, kernel_size=3, padding=1, bias=False, r=8)
        self.bn2 = nn.BatchNorm2d(width // 2)
        self.conv3 = lora.Conv2d(width // 2, width, kernel_size=3, padding=1, bias=False, r=8)
        self.bn3 = nn.BatchNorm2d(width)
        self.avgpool = nn.AvgPool2d(2)
        self.relu = nn.ReLU(inplace=True)
    ...
    def _make_layer(self, planes, blocks, stride=1):
        layers = [BottleneckLora(self._inplanes, planes, stride)]

        self._inplanes = planes * BottleneckLora.expansion
        for _ in range(1, blocks):
            layers.append(BottleneckLora(self._inplanes, planes))
    ...
    def forward(self, x):
        ...
        x = x.type(self.conv1.conv.weight.dtype)
        x = stem(x)
