...
class DETRPanopticCaptionDesDatasetMapper:
    """
    ...
    """
    COCO_DES_PATH = os.path.join(os.getenv("DETECTRON2_DATASETS", "datasets"), "coco", 'coco_opt_greedy_v1.json')
    ...
    def __init__(
        ...
        *,
        ...
        for dataset in dataset_list:
            ...
            with open(self.COCO_DES_PATH) as f:
                des_data = json.load(f)

            cls_name_des = [id2name[x] for x in range(len(id2name))]
            cls_name_p = process_coco_cat(cls_name_des)
            for c_n, c_n_p in zip(cls_name_des, cls_name_p):
                des = des_data[c_n]
                if len(des) == 0:
                    des = ['']
                des = [c_n_p + " " + d for d in des]
                c_t = tokenize(des, context_length=77)
                self.id2desList.append(c_t)
    ...
    def __call__(self, dataset_dict):
        ...
        if self.per_region_super:
            class_name = torch.clone(self.dataset2token[dataset_dict.pop('dataset')])
            num_cat = len(self.id2desList)
            cat_index = np.random.rand(num_cat)
            description = torch.stack([des[int(idx * len(des))] for des, idx in zip(self.id2desList, cat_index)])

            des_idx = np.random.rand(num_cat) > 0.5
            class_name[des_idx] = description[des_idx]
            dataset_dict['all_caption'] = class_name
        # import pdb;pdb.set_trace()
