...
class OpenSegMaskFormerDenseTextKDentropy(nn.Module):
    """
    ...
    def from_config(cls, cfg):
        ...
        dense_list = ['loss_mask', 'loss_dice' ]
    ...
    def forward(self, batched_inputs):
        ...
        # for entropy
        # indices_to_remove = [0, 13, 64, 85]

        # new_tensor = torch.cat((plain_class_ebmd[:, :indices_to_remove[0]], plain_class_ebmd[:, indices_to_remove[0]+1:]), dim=1)
        # for idx in indices_to_remove[1:]:
        #     new_tensor = torch.cat((new_tensor[:, :idx], new_tensor[:, idx+1:]), dim=1)
        
        # plain_class_ebmd = new_tensor
        # import pdb; pdb.set_trace()
