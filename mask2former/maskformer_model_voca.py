...
class OpenSegMaskFormer(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        attn_shape = self.backbone.output_shape()['attn'] if 'attn' in self.backbone.output_shape() else 0
        vision_in_dim = {
            "resAtten": attn_shape,
    ...
    def from_config(cls, cfg):
        ...
        use_neg_samples = cfg.INPUT.DATASET_MAPPER_NAME == "detr_panoptic_caption_neg"
    ...
    def forward(self, batched_inputs):
        ...
        # # flops1 = flop_count_operators(self.backbone, images.tensor)
        # flops11 = FlopCountAnalysis(self.backbone, images.tensor)
        # print(flops11.total())
        # # import pdb; pdb.set_trace()
        # param1 = parameter_count_table(self.backbone)
        # print(param1)
        # # import pdb; pdb.set_trace()
        
        import pdb; pdb.set_trace()
        # self.backbone.load_state_dict(torch.jit.load("pretrained/RN50.pt"))
        features = self.backbone(images.tensor)
        torch.save(features, "han-subnetwork.pt")
        import pdb; pdb.set_trace()
        # print(images.tensor.shape)
        # # features = self.backbone(batched_inputs)
        # flops2 = flop_count_operators(self.sem_seg_head, features)
        # flops22 = FlopCountAnalysis(self.sem_seg_head, features)
        # print(flops22.total())
        # param2 = parameter_count_table(self.sem_seg_head)
        # print(param2)
        # import pdb; pdb.set_trace()
        ...
        if self.grounding_super:        
            ...
            # flops3 = flop_count_operators(self.text_encoder, text)
            # print(flops3)
            # param3 = parameter_count_table(self.text_encoder)
            # print(param3)
        if self.region_supervision:     
            all_class = torch.stack([x["all_caption"].to(self.device) for x in batched_inputs])
            all_class_ebmd = self.text_encoder(all_class)
            # flops4 = flop_count_operators(self.text_encoder, all_class)
            # print(flops4)
            # param4 = parameter_count_table(self.text_encoder)
            # print(param4)
        ...
        if self.training:
            ...
            # outdir = '/opt/data/private/xjx/data/sup/pc-459-hans'
            processed_results = []
    ...
    def loss_labels(self, src_logits, targets, indices, ignore_masks):
        ...
        # import pdb;pdb.set_trace()
        if self.use_neg_samples: # neg samples
            neg_class_num = src_logits.shape[2] - self.criterion.empty_weight.shape[0]
            class_weight = torch.cat([self.criterion.empty_weight, torch.ones(neg_class_num).to(self.device) * 0.5])
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, class_weight)
        else:
