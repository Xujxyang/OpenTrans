...
class AttentionPool2dMasked(nn.Module):
    ...
    def forward(self, x, mask):
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]).permute(2, 0, 1)  # NCHW -> (HW),N,C
        mask = mask.reshape(mask.shape[0], mask.shape[1], mask.shape[2] * mask.shape[3]).permute(2, 0, 1) # (HW),N,C
        x = x * mask
...
class CLIPResNetWithAttention(Backbone):
    ...
    def forward(self, x):
        ...
        if self.attnpool is not None:
            ...
            outs['attn_g'] = x_global
            outs['attn_l'] = x_local

        return outs
    
    def forward_attn(self, x, mask):
        # x: (1,c,h,w),  mask:(n,1,h,w)
        n, *_ = mask.shape
        _, c, *_ = x.shape
        mask = mask.float()
        mask = F.interpolate(mask, scale_factor=1/32, mode='nearest')
        x_global, x_local = self.attnpool(x.repeat(n,1,1,1), mask.repeat(1,c,1,1))
        return x_global
...
class CLIPVisionTransformer(Backbone):
    ...
    def forward(self, x: torch.Tensor, attn_mask, max_num_obj):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], max_num_obj, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]

        pos = self.positional_embedding.to(x.dtype)
        # import pdb;pdb.set_trace()
        pos = torch.cat([pos[0] + torch.zeros(max_num_obj - 1, x.shape[-1], dtype=x.dtype, device=x.device), pos], dim=0)
        ...
        x, _ = self.transformer(x, attn_mask)
