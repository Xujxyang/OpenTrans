...
class OpenSegMaskFormerDenseTextKDSyn(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.plain_class_super: #coco 加监督
        ...
        # vision feature的提取方式，基本上使用queryclass
        ...
        if self.training:
            ...
            if self.plain_class_super:
                ...
                # vision (bsz, 100, dim_c), text (bsz, 133+1, dim_c)
                # similarity (bsz, 100, 133+1)
                # dim_c -> 133+1
                #加入背景类
                ...
                plain_class_ebmd = plain_class_ebmd.permute(0, 2, 1) #与vision embedding对齐
                cls_pred = torch.bmm(vision_ebmd_norm * logit_scale, plain_class_ebmd) #vision,text相似性输出
            ...
            losses, indices, aux_indices = self.criterion(outputs, targets, get_all_indice=True)#ce_loss
