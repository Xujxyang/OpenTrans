...
class CLIP(Backbone):
    def __init__(self, cfg, input_shape):
        super().__init__()
        model_name = cfg.MODEL.FC-CLIP.CLIP_MODEL_NAME
        pretrained= cfg.MODEL.FC-CLIP.CLIP_PRETRAINED_WEIGHTS
        ...
        # pruning.see_zero_rate(self.clip_model.visual)
        ...
        # mask_dict_a={key:mask_dict_a[key].cpu() for key in mask_dict_a}
        # pruning.imagenet_pruning_model_custom_res50v1(self.clip_model.visual, mask_dict_a)
        # pruning.see_zero_rate(self.clip_model.visual)
        ...
        self.freeze_token_embedding()
        self.freeze_other()
    # import pdb; pdb.set_trace()
    def freeze_head(self):
        for param in self.clip_model.visual.head.parameters():
            param.requires_grad = False
        for param in self.clip_model.visual.trunk.head.parameters():
            param.requires_grad = False

    def freeze_attnpool(self):
