...
class OpenSegMaskFormerDense(nn.Module):
    """
        ...
        if self.training:
            ...
            if self.region_supervision:
                ...
                for i, aux_query in enumerate(seg_feature['query_feature']):
                    aux_query = self.embed_proj(aux_query)
                    aux_query = aux_query / (aux_query.norm(dim=-1, keepdim=True) + 1e-7)
                    aux_cls_pred = torch.bmm(aux_query * logit_scale, all_class_ebmd)
    ...
    def semantic_inference(self, mask_cls, mask_pred):
        ...
        #         imageio.imwrite("./tmp/{}/seg_{}_{}.png".format(path, i, VSPW_CATEGORIES[i+1]['name']), np.array(seg_i.cpu()))
