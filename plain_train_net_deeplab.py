...
from mask2former import (
    ...
    DeeplabPanopticCaptionKDDatasetMapper
)
...
def build_train_loader(cfg):
    ...
    # coco panoptic segmentation lsj new baseline
        ...
        return build_detection_train_loader(cfg, mapper=mapper)
    elif cfg.INPUT.DATASET_MAPPER_NAME == "deeplab_panoptic_caption_syn_kd":
        mapper = DeeplabPanopticCaptionKDDatasetMapper(cfg, True)
        return build_detection_train_loader(cfg, mapper=mapper)
    else:
        mapper = None
        return build_detection_train_loader(cfg, mapper=mapper)
...
def do_train(cfg, model, resume=True):
    ...
    imp_num = 0
    a = 0
    OUTDIR = 'seed{}_imp_deeplab_outbackbone'.format(random_seed)
    if not os.path.exists(OUTDIR): os.makedirs(OUTDIR)

    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            # #continue fintuning(load-mask)
            # if a == 0:
            #     # import pdb; pdb.set_trace()
            ...
            #     pruning.imagenet_pruning_model_custom_res50v1(model.module.backbone, mask_dict_a)
            #     # import pdb; pdb.set_trace()
            #     pruning.see_zero_rate(model.module.backbone)
            #     a = 1

            # if iteration == 1:
            #     load_name = './pruned_mask/seg_backbone_imp{}.pth'.format(21)
            ...
            #     pruning.imagenet_remove_model_custom_res50v1(model.module.backbone, False)
            ...
            #     pruning.imagenet_remove_model_custom_res50v1(model.module.backbone, False)
            #     # import pdb;pdb.set_trace()
            #     # pruning.remove_prune(model.backbone.layer4[2].conv3, False)
            #     # print('i want')
            #     pruning.see_zero_rate(model.module.backbone)
            #     save_name_2 = 'seg_clip_imp{}'.format(imp_num)
            #     checkpointer2 = DetectionCheckpointer(model, save_dir="deeplab_10")
